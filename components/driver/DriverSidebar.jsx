...
import { Home, Package, Clock3, User, Wallet } from "lucide-react";
          ...
          {links.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className="relative flex h-full min-w-0 flex-col items-center justify-center"
              >
                ...
                <div
                  className={`flex flex-col items-center gap-1 transition-all duration-200 ${isActive ? "translate-y-[-2px]" : ""}`}
                >
                  <Icon
                    size={22}
                    strokeWidth={isActive ? 2.5 : 2}
