...
export default function LocationSearchPage() {
  ...
  // --------------------------------------------------
  // CREATE AUTOCOMPLETE SESSION TOKEN
  // --------------------------------------------------
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        ...
        /*
         * Bias search toward currently selected
         * delivery location.
         *
         * Example:
         * customer selected Home in Nandurbar,
         * therefore searching "Shivaji" should
         * prefer results around that area.
         */
        if (
          customerLocation?.latitude != null &&
          customerLocation?.longitude != null
        ) {
          payload.latitude = customerLocation.latitude;

          payload.longitude = customerLocation.longitude;
        }
        ...
        /*
         * Ignore an older API response if the user
         * has already typed another search.
         */
  ...
  const handleSelectPrediction = async (prediction) => {
    ...
    try {
      setSelectedPrediction(prediction.placeId);

      setError("");

      /*
       * We don't select the location yet.
       *
       * First send the customer to the map
       * so they can verify/adjust the exact pin.
       */
      ...
      const params = new URLSearchParams({
        ...
        label: prediction.mainText || prediction.text || "Selected Location",
      });
  ...
  const handleUseCurrentLocation = async () => {
    try {
      ...
      /*
       * We only obtain the GPS location here.
       *
       * After this, send the coordinates to
       * /location/map where we will show the map,
       * reverse-geocode the area and allow confirmation.
       */
            ...
            {!searchLoading && predictions.length === 0 ? (
              ...
              >
                ...
                <p className="mx-auto mt-2 max-w-sm text-xs leading-relaxed text-white/30">
                  Try searching for an area, landmark, road or building name.
                </p>
              </motion.div>
            ) : (
                ...
                <AnimatePresence initial={false}>
                  {predictions.map((prediction, index) => (
                    <motion.button
                      ...
                      disabled={selectedPrediction === prediction.placeId}
                    ...
                    >
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-cyan-500/10 text-cyan-400">
                        {selectedPrediction === prediction.placeId ? (
