                ...
                <div className="overflow-x-auto max-w-4xl rounded-md shadow border border-gray-200">
                    <table className="w-full text-sm text-left text-gray-600">
                        <thead className="bg-gray-50 text-gray-700 text-xs uppercase tracking-wider">
                            <tr>
                                {["Sr. No.", "Customer", "Total", "Payment", "Coupon", "Status", "Actions", "Date"].map((heading, i) => (
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {orders.map((order, index) => (
                                <tr key={order.id} className="hover:bg-gray-50 transition-colors duration-150 cursor-pointer">
                                    ...
                                    <td className={`px-4 py-3 font-medium text-slate-800 ${order.status === 'CANCELLED' ? 'text-red-600' : ''}`}>
                                        {order.status === 'CANCELLED' ? `-₹${order.total}` : `₹${order.total}`}
                                    </td>
                                    <td className="px-4 py-3">{order.paymentMethod}</td>
                                    <td className="px-4 py-3">{order.isCouponUsed ? order.coupon?.code : '—'}</td>
                                    <td className="px-4 py-3">
                                        <span className={`${order.status === 'CANCELLED' ? 'text-red-600 font-bold' : ''}`}>
                                            {order.status}
                                        </span>
                                    </td>
                ...
                <div onClick={closeModal} className="fixed inset-0 flex items-center justify-center bg-black/50 text-slate-700 text-sm backdrop-blur-xs z-50">
                    <div onClick={e => e.stopPropagation()} className="bg-white rounded-lg shadow-lg max-w-2xl w-full p-6 relative">
                        ...
                        <div className="mb-4">
                            ...
                            {selectedOrder.isCouponUsed && <p><span className="text-green-700">Coupon:</span> {selectedOrder.coupon.code} ({selectedOrder.coupon.discount}% off)</p>}
                            ...
                            {selectedOrder.status === 'CANCELLED' && <p className="text-red-600 font-semibold mt-2">Canceled Amount: -₹{selectedOrder.total}</p>}
                        </div>
